...
def weighted_sampler(df: pd.DataFrame):
    """ Weighted Random Sampelr for imbalanced Dataset """
    label_dict = {"CE": 0, "LAA": 1}
    label_number = df['label'].value_counts().to_list()
    total_number = sum(label_number)
    labels = df['label'].to_list()

    class_weights = [total_number / label_number[i] for i in range(len(label_number))]
    weights = [class_weights[label_dict[labels[i]]] for i in range(int(total_number))]
